...
const TodoList = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'active' | 'completed' | 'today' | 'overdue'>('today')
  ...
  const filteredTasks = useMemo(() => {
    ...
    switch (filter) {
      ...
      case 'overdue':
        filtered = filtered.filter(task =>
          task.due_date && isAfter(new Date(), task.due_date) && !isToday(task.due_date) && task.status !== 'Done'
        )
        break
    }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      overdue: areaFilteredTasks.filter(t => t.due_date && isAfter(new Date(), t.due_date) && !isToday(t.due_date) && t.status !== 'Done').length,
    }
  }, [tasks, selectedArea])
